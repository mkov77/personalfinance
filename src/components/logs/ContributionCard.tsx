...
import { Card, Text, Badge, Button, Group, Menu, ActionIcon, rem } from '@mantine/core';
  ...
  return (
    <Card shadow="sm" padding="sm" radius="md" withBorder key={contribution.contributionid} style={{ marginTop: '10px' }}>
      <Group justify="space-between" mt="md" mb="xs">
        <Text fw={500}>{contribution.contributor}</Text>
        {renderMenu()}
      </Group>

      <Text size="sm" c="dimmed">
        Date: {new Date(contribution.contributiondate).toLocaleDateString()}<br />
        Amount: ${Number(contribution.contributionamount).toFixed(2)}<br />
        Goal: {contribution.contributiongoal}
      </Text>
      <Badge color="pink">{contribution.contributionsource}</Badge>
    </Card>
  );
};
